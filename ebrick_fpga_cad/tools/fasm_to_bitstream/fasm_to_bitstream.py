...
def generate_umi_bitstream(bitstream_map,
    ...
    if ((config_data_width != 8) and (config_data_width != 16) and (config_data_width != 32)):
        exit()

    umi_bitstream = []

    if (umi_rom_data_width > config_data_width):
        addresses_per_rom_address = int(math.ceil(umi_rom_data_width / config_data_width))
    else:
        addresses_per_rom_address = 1

    config_words_per_address = int(umi_rom_data_width / config_data_width)

    umi_addresses_per_row = int(math.ceil(num_bitstream_columns / addresses_per_rom_address))

    num_column_slots = umi_addresses_per_row * addresses_per_rom_address
    ...
    if (reverse):
        start_address = bitstream_size - addresses_per_rom_address
        stop_address = -1
        address_increment = -1 * addresses_per_rom_address * umi_addresses_per_row
    else:
        start_address = 0
        stop_address = bitstream_size
        address_increment = addresses_per_rom_address * umi_addresses_per_row
...
def calculate_bitstream_size(bitstream_map, num_column_slots):
    ...
    bitstream_size = len(bitstream_map) * num_column_slots
    bitstream_size *= get_max_bitstream_address_length(bitstream_map)
